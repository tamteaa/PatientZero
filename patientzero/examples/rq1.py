"""RQ1: does optimization improve outcomes for LOW-literacy patients?

Self-contained. Pins patient literacy to "low"; all other traits (age,
...
Run with:
    uv run python -m patientzero.examples.rq1
"""
...
# ── Patient distribution — literacy pinned to "low" ───────────────────────────
...
LOW_LITERACY_PATIENT = _PATIENT.reweight("literacy", {"low": 1.0})
...
RQ1_CONFIG = ExperimentConfig(
    name="rq1-low-literacy",
    agents=(
        Agent("doctor",  DOCTOR_TEMPLATE,  DOCTOR),
        Agent("patient", PATIENT_TEMPLATE, LOW_LITERACY_PATIENT),
    ),
...
SIMS_PER_ROUND = 10
RESULTS_PATH = Path(__file__).parent / "results_rq1.json"
...
async def main() -> None:
    ...
    try:
        ...
        config = replace(RQ1_CONFIG, name=f"{RQ1_CONFIG.name}-{stamp}")
