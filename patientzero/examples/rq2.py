"""RQ2: does optimization improve outcomes for HIGH-literacy patients?

Self-contained. Pins patient literacy to "high"; all other traits (age,
education, anxiety, tendency, scenario) still sample from the causal DAG.

Paper protocol:
    model=kimi:kimi-k2.5, max_turns=6, seed=42, n=10 per round,
    num_optimizations=1 (baseline round + one optimized round).

The seed matches RQ1 so the shared random draws are comparable across
cohorts when only literacy differs.

Run with:
    uv run python -m patientzero.examples.rq2
"""
...
# ── Patient distribution — literacy pinned to "high" ──────────────────────────
# Full DAG preserved; only the literacy node is reweighted to a single value.

_PATIENT = Distribution(
...
HIGH_LITERACY_PATIENT = _PATIENT.reweight("literacy", {"high": 1.0})
...
RQ2_CONFIG = ExperimentConfig(
    name="rq2-high-literacy",
    agents=(
        Agent("doctor",  DOCTOR_TEMPLATE,  DOCTOR),
        Agent("patient", PATIENT_TEMPLATE, HIGH_LITERACY_PATIENT),
    ),
...
SIMS_PER_ROUND = 10
RESULTS_PATH = Path(__file__).parent / "results_rq2.json"
...
async def main() -> None:
    ...
    try:
        ...
        config = replace(RQ2_CONFIG, name=f"{RQ2_CONFIG.name}-{stamp}")
        exp = await Experiment.create(config, db=db)

        print(f"experiment: {exp.id}  name: {config.name}")
        print(f"rounds: {1 + config.num_optimizations}  sims/round: {SIMS_PER_ROUND}")
