"""
Experiment — user-facing facade over an ExperimentConfig + RepoSet.

One path for creation:

    config = ExperimentConfig(name=..., agents=(...), judge=..., model=...)
    exp = Experiment(config, repos)
    await exp.run(n=5)

``Experiment.load(name, repos)`` re-opens an existing row by name.
"""
...
class Experiment:
    ...
    def __init__(self, config: ExperimentConfig, repos: RepoSet):
        self._repos = repos
    ...
    @classmethod
    def load(cls, name: str, repos: RepoSet) -> "Experiment":
