...
def test_complete_simulation(db):
    sim = create_simulation(db, "Maria", "CBC", "clinical", "static", "mock:default", {})
...
def test_add_and_get_turns(db):
    sim = create_simulation(db, "Maria", "CBC", "clinical", "static", "mock:default", {})
    add_simulation_turn(db, sim["id"], 0, "explainer", "ExplainerAgent", "Hello", 100.0)
    add_simulation_turn(db, sim["id"], 1, "patient", "PatientAgent", "Hi", 200.0)

    turns = get_simulation_turns(db, sim["id"])
    assert len(turns) == 2
    assert turns[0]["role"] == "explainer"
    assert turns[0]["agent_type"] == "ExplainerAgent"
...
def test_list_simulations(db):
    create_simulation(db, "Maria", "CBC", "clinical", "static", "mock:default", {})
    create_simulation(db, "James", "HbA1c", "analogy", "dialog", "mock:default", {})
